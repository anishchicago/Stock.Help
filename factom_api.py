...
KEY = '87db77640a6a4a8a25fe5316d8183006'
...
def _decode_response(data):
    res = json.loads(data.decode(chardet.detect(data)["encoding"]))
...
def create_chain(external_ids=None, content=None):
    """ """
...
def chain_add_entry(chain_id=None, external_ids=None, content=None):
    """ """
    _ids = [ _encode(extid) for extid in external_ids ]
    payload = json.dumps({"external_ids": _ids, "content": _encode(content) })
